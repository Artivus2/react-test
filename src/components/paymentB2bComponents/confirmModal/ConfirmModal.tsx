...
import { BuyerB2bPay } from '../../../services/P2p';
...
const ConfirmModal = ({
  ...
  const sendFile = async () => {
    pubnub
      //@ts-ignore
      .sendFile({ channel: `b2b_order_${id}${date.split(" ").join('_').split("-").join("_").split(":").join("_")}`, file: photoList, message: '' })
      .then(res => {
        setLoading(false)
  ...
  const goNext = async () => {
    if (checkedInput && photoList) {
      const token = localStorage.getItem("access_token") ?? '';
      const data = await BuyerB2bPay(token, id);
      if (data.status === 200) {
        sendFile()
      } else {
        setLoading(false)
